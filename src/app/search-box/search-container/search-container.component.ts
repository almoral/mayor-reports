...
export class SearchContainerComponent implements OnInit, OnDestroy {
    ...
    @Output() onSetMonthFilter = new EventEmitter();
    @Output() onSetYearFilter = new EventEmitter();
    ...
    ngOnInit() {
        ...
        this.selectedFilters$ = combineLatest(this.currentSelectedTypes$, this.currentSelectedCategories$).pipe(
    ...
    removeFilter(searchTerm: string) {
        this.currentSelectedTypes$
            .pipe(map((types) => _.difference(types, [ searchTerm ])), take(1))
            .subscribe((value) => {
                this.onSetYearFilter.emit(value);
            });

        this.currentSelectedCategories$
            .pipe(map((categories) => _.difference(categories, [ searchTerm ])), take(1))
            .subscribe((value) => {
                this.onSetMonthFilter.emit(value);
            });
    }
    ...
    onYearSelected(options: string[]) {
        this.onSetYearFilter.emit(options);
    }

    onMonthSelected(options: string[]) {
        this.onSetMonthFilter.emit(options);
    }

    //  This method is used by the search button and pressing enter on the textbox when liveFilter is set to false.
    updateTitleFilter() {
        this.searchTerm$.pipe(take(1)).subscribe((searchTerm) => this.onSetTitleFilter.emit(searchTerm));
        this.clearFilters();
    }
    ...
    clearFilters() {
        this.onSetMonthFilter.emit([]);
        this.onSetYearFilter.emit([]);
    }
}
