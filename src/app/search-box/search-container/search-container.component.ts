...
export class SearchContainerComponent implements OnInit, OnDestroy {
    constructor(private monthService: MonthService, private dataStoreService: DataStoreService) {}
    ...
    @Input() currentSelectedYearSubject = new BehaviorSubject([]);

    @Input() currentSelectedMonthSubject = new BehaviorSubject([]);
    ...
    ngOnInit() {
        ...
        this.selectedFilters$ = combineLatest(this.currentSelectedYear$, this.currentSelectedMonth$);
    }
    ...
    removeFilter(searchTerm: string) {
        const types = this.currentSelectedYearSubject.getValue();
        this.setYearFilter.emit(_.map((types) => _.difference(types, [ searchTerm ])));

        this.currentSelectedMonthSubject
